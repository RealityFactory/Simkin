...
// Create an object to read the script file in
  ...
  in->open(KFileName);
  if (in->eof()==false){
// Create a reader to read a TreeNode in
    skTreeNodeReader * reader=new (ELeave)skTreeNodeReader;
    CleanupStack::PushL(reader);
// Try to read in the script
    skTreeNode * node=reader->read(in,KFileName,*ctxt);
    if (node){
      CleanupStack::PushL(node);
      // Create an executable object to wrap the script
      skTreeNodeObject * obj=new (ELeave)skTreeNodeObject;
      CleanupStack::PushL(obj);
      obj->setNode(KFileName,node,false);
